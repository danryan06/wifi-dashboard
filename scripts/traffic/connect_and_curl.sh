...
# Make ERR trap propagate into functions/subshells
set -o errtrace

# Log failures instead of exiting; keep the service alive
trap 'ec=$?; echo "[$(date "+%F %T")] TRAP-ERR: cmd=\"$BASH_COMMAND\" ec=$ec line=$LINENO" | tee -a "$LOG_FILE"' ERR

# Enhanced Wi-Fi Good Client Simulation with BSSID Roaming - SYNTAX FIXED VERSION
...
set +e  # Changed from set -E which was causing issues
trap 'ec=$?; echo "[$(date "+%F %T")] TRAP-ERR: cmd=\"$BASH_COMMAND\" ec=$ec line=$LINENO" | tee -a "$LOG_FILE"' ERR
...
# Global roaming state - Fixed associative array declarations
...
freqs_for_band() {
  case "${1:-2.4}" in
    ...
    both) 
        echo "2412 2417 2422 2427 2432 2437 2442 2447 2452 2457 2462 2467 2472 5180 5200 5220 5240 5260 5280 5300 5320 5500 5520 5540 5560 5580 5600 5620 5640 5660 5680 5700 5720 5745 5765 5785 5805 5825"
        ;;
    *) echo "2412 2417 2422 2427 2432 2437 2442 2447 2452 2457 2462 2467 2472";;
  esac
}

# Fixed BSSID discovery function
discover_bssids_for_ssid() {
  ...
  # Fixed: Use temporary file instead of complex process substitution
  local temp_scan="/tmp/wifi_scan_$$"
  
  # Primary path: iw scan with file processing
  if sudo iw dev "$INTERFACE" scan > "$temp_scan" 2>/dev/null; then
    # Process the scan results with awk
    awk -v ss="$target_ssid" -v band="$WIFI_BAND_PREFERENCE" '
        ...
        }' "$temp_scan" | sort -k2,2nr > "${temp_scan}.results"
    ...
    done < "${temp_scan}.results"
    
    rm -f "$temp_scan" "${temp_scan}.results"
  fi

  # Fallback: nmcli if iw yielded nothing
  if [[ ${#DISCOVERED_BSSIDS[@]} -eq 0 ]]; then
    local nmcli_temp="/tmp/nmwifi_$$"
    if nmcli -t --separator '|' -f SSID,BSSID,FREQ,SIGNAL dev wifi list ifname "$INTERFACE" > "$nmcli_temp" 2>/dev/null; then
      while IFS='|' read -r ssid bssid freq sig; do
        ...
        # Convert nmcli % to rough dBm for thresholding
        local dbm=$(( sig / 2 - 100 ))
        if (( dbm >= MIN_SIGNAL_THRESHOLD )); then
          DISCOVERED_BSSIDS["$bssid"]="$ssid"
          BSSID_SIGNALS["$bssid"]="$dbm"
          log_msg "(nmcli) Found BSSID: $bssid (approx ${dbm} dBm @ ${freq} MHz)"
        fi
      done < "$nmcli_temp"
      rm -f "$nmcli_temp"
    fi
...
select_roaming_target() {
    ...
    for bssid in "${!DISCOVERED_BSSIDS[@]}"; do
        local signal="${BSSID_SIGNALS[$bssid]:--100}"
...
perform_roaming() {
    ...
    if timeout "$CONNECTION_TIMEOUT" nmcli connection up "$roam_connection_name" >/dev/null 2>&1; then
        ...
        if [[ "$new_bssid" == "$target_bssid" ]]; then
            ...
            log_msg "Roaming Event Complete: Signal strength ${BSSID_SIGNALS[$target_bssid]}dBm"
...
connect_to_wifi_with_roaming() {
    ...
    # Method 1: Create connection profile with specific BSSID
    ...
    if [[ "$connection_success" == "true" ]]; then
        ...
        if [[ "$ip_assigned" != "true" ]]; then
            log_msg "Connected but no IP address assigned after 40s, attempting recovery..."
            
            # Use NetworkManager for DHCP recovery
...
main_loop() {
    ...
    while true; do
        ...
        if [[ "$state" == "100" && -n "$ip_addr" && "$current_ssid" == "$SSID" ]]; then
            ...
            log_msg "Connection issue detected:"
            ...
            if [[ "$state" != "100" ]]; then
                ...
                # Use NetworkManager IP recovery
                nmcli device reapply "$INTERFACE" 2>/dev/null || true
                sleep 5
